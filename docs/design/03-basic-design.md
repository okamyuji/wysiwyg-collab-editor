...
本書のMVPはフェーズ区分により段階化する。2026-07-05のユーザー決定(ADR-0029)により、現行実装(in-memory WebSocketリレーとcontentEditableによる簡易同期、永続化なし)をフェーズ1のMVPとして正式化した。本書の「MVP」の語のうち、OT、ShareDB、Quill.js、PostgreSQL永続化、Redis Pub/Sub、S3互換オブジェクトストレージ、日本語IME確定前ガード、性能予算、カバレッジ閾値ゲートに依存する記述は、フェーズ2以降に適用する。フェーズ区分の確定表は要件定義書 第8章のフェーズ区分表を正本とする。
